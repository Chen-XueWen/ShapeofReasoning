...
"""
Analyze association between graph connectivity features and alignment score using OLS.

Configuration:
- Dependent variable: score
- Predictors: ordered set of graph features (see GRAPH_FEATURE_COLUMNS)
- Controls: coverage, n_pairs (if available in alignment data; not included by default in OLS)
...
def run_analysis(df: pd.DataFrame, dest: Path, label: str) -> None:
    ...
    available_features = [col for col in GRAPH_FEATURE_COLUMNS if col in df.columns]
    ...
    X = sm.add_constant(data[available_features], has_constant="add").astype(float) #need this as some values are integer
    y = data["score"]
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Analyze graph feature influence on alignment score (OLS)")
    ap.add_argument("--align", default="data/aime_align_dp/gpt-oss_20b", help="Alignment JSONL file or directory")
    ap.add_argument("--features", default="data/aime_graph/trace/gpt-oss_20b.jsonl", help="Graph feature JSONL (trace split)")
    ap.add_argument(
        "--outdir",
        default="analysis/gpt-oss_20b/graph_vs_alignment",
