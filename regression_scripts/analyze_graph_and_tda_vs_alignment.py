...
def load_dataset(path: Path) -> pd.DataFrame:
    ...
    numeric_candidates = [
        ...
        or col.startswith("graph_")
        or col.startswith("align_")
    ]
...
def run_analysis(
    ...
    if not available_features:
        print(f"[{label}] none of the configured features are present; skipping.")
        return
...
def main() -> None:
    ...
    if not args.skip_overall:
        ...
        run_analysis(df_overall, combined_feature_columns, dest, label)
