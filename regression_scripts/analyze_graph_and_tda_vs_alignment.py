...
def load_data(align_path: Path, features) -> pd.DataFrame:
...
def create_features(graph_path: Path, tda_path: Path):
    df_graph = pd.read_json(graph_path, lines=True)
    df_tda = pd.read_json(tda_path, lines=True)

    df_graph = df_graph.drop('score', axis=1)
    df_tda = df_tda.drop('score', axis=1)
    df_graph["id"] = df_graph["id"].astype(str)
    df_tda["id"] = df_tda["id"].astype(str)

    df = df_tda.merge(df_graph, on="id", how="inner")
    return df
...
def main() -> None:
    ...
    ap.add_argument("--featuresTDA", default="data/processed/gpt-oss_120b_aime2022/tda_features.jsonl")
    ap.add_argument("--featuresGraph", default="data/processed/gpt-oss_120b_aime2022/graph_features.jsonl")
    ap.add_argument("--outdir", default="data/processed/analysis/gpt-oss_120b_aime2022_tda_and_graph")
    ...
    features = create_features(Path(args.featuresGraph), Path(args.featuresTDA))

    df = load_data(Path(args.align), features)
    ...
    feat_cols = [
        ...
        if c not in {"id", "score", "coverage", "n_pairs", "indices", "H0_betti_location", "H0_betti_peak", "H0_betti_trend", "H1_betti_trend", "H0_mean_life", "H0_skewness", "H1_mean_life", "H1_skewness", "H0_max_birth", "H0_max_death", "H1_max_birth", "H1_max_death", "H0_landscape_mean","H0_landscape_max", "H0_landscape_area", "H1_landscape_mean", "H1_landscape_max", "H1_landscape_area", "has_loop"}
