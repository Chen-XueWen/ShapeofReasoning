...
def betti_curve(diagrams: dict[int, np.ndarray], dim: int = 0) -> tuple[np.ndarray, np.ndarray]:
    """
...
def betti_curve_summary_features(
    diagrams: dict[int, np.ndarray], dim: int = 0
) -> dict[str, float]:
    """
    ...
    ts, bc = betti_curve(diagrams, dim=dim)
