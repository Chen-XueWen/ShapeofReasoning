...
def load_data(align_path: Path, features_df) -> pd.DataFrame:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Analyze TDA feature influence on sample score (OLS)")
    ap.add_argument("--align", default="data/processed/alignments_sampling_scores.jsonl")
    ap.add_argument("--features", default="data/processed/gpt-oss_120b/tda_features.jsonl")
    ap.add_argument("--outdir", default="data/processed/analysis/sampling")
    ...
    #calculating average features over n runs
    ...
    for i in range(1, n+1):
        ...
        if i == 1:
            features_df = features_i
        else:
            features_df = features_df.add(features_i, fill_value=0)
    
    features_df = features_df/n



    features_df = features_df.reset_index()
    df = load_data(Path(args.align), features_df)
